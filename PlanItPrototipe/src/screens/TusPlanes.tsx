...
export default function TusPlanes() {
  ...
  function handlePlanTap(plan: Plan) {
    if (plan.type === "planGanador") {
      navigate("/plan-ganador?mode=vote");
    } else {
      navigate(plan.status === "confirmed" ? "/plan?confirmed=1" : "/plan");
    }
  }
      ...
      <main className="pt-[80px] pb-[88px] px-6 max-w-[640px] mx-auto">
        ...
        <div className="mt-6">
          <button
            ...
            className="w-full bg-primary text-on-primary py-4 rounded-2xl text-[15px] font-extrabold active:scale-[0.98] transition-transform"
          >
            Crear Juntada
          </button>
        </div>
